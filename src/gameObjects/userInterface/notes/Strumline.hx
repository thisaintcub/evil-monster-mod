...
class Strumline extends FlxTypedGroup<FlxBasic>
{
	...
	public function new(x:Float = 0, playState:PlayState, ?character:Character, ?displayJudgements:Bool = true, ?autoplay:Bool = true, ?keyAmount:Int = 4, 
		?downscroll:Bool = false, ?parent:Strumline)
	{
		super();
		...
		{
			var staticArrow:UIStaticArrow = ForeverAssets.generateUIArrows(-25 + x, 10 + (downscroll ? FlxG.height - 200 : 0), i, PlayState.assetModifier);
			staticArrow.ID = i;

			// i dont give a shiiiiiiiiit i dont have time
			switch(i)
			{
				case 0:
					staticArrow.x -= 30;
			...
			staticArrow.setGraphicSize(staticArrow.width * 0.55);
